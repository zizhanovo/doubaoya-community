...
def test_notice_is_read_and_surfaced(rel: str) -> None:
    """读出来还不够，必须真的送到用户眼前。"""
    src = _read(rel)
    # ① 读：从信封上取 notice（env.notice / j.notice / rendered.notice 都算）。
    assert re.search(r"\.notice\b", src), (
        f"{rel} 一次都没读过 notice —— 服务端挂了没人读，等于没挂。"
        f"样板见 skills/dby-api/scripts/doubaoya.mjs"
    )
    # ② 送：要么打给用户（stderr / 回报），要么原样带出给上层。
    # 两种合格的「送出去」：打给用户（stderr），或原样放进返回值交给上层去打。
    surfaced = re.search(r"console\.error\([^)]*notice", src) or re.search(r"^\s*notice:", src, re.M)
    assert surfaced, (
...
def test_no_envelope_reader_is_missing_from_the_list() -> None:
    ...
    """
    suspects = []
    for p in sorted((ROOT / "skills").rglob("*.mjs")):
        rel = p.relative_to(ROOT).as_posix()
        if rel in ENVELOPE_READERS:
            continue
        src = p.read_text(encoding="utf-8")
        # 自己判 success 的 = 自己在解析信封的
        if re.search(r"\b(?:if\s*\(\s*!?\w+\.success|\w+\.success\s*(?:!==|===))", src):
            suspects.append(rel)
    assert not suspects, (
