...
# 合规能力与封面套路数据并进 `dby-api` 按意图路由。元组再次只留**下架后仍存在**的那几跳；
# 「合规环与封面套路数据必须有人接」这两条不变量改由下面 CHAIN_CAPABILITIES 守。
# 成品图生成当前暂时下架，不得为了补齐链路而要求不存在的 capability；未来是否恢复需重新评估，
# 当前不承诺恢复时间。
AUTHORING_CHAIN = (
    "dby-theme",
    "dby-publish",
...
# 已下架包的能力**当前也不在发现面**的，列在这儿豁免「删包前能力必须仍可发现」那道闸。
...
RETIRED_WITH_CAPABILITY = {
    ...
    "mera",
    # 出图能力当前暂时下架，不在发现接口与能力索引中；未来是否恢复需重新评估，
    # 当前不承诺恢复时间。哪天路由真回到能力索引，下方的自动清账断言会要求删掉这条。
    "image-gen",
...
# 能力**本身**也当前不在架的包，触发词不该迁——迁了等于承诺一个当前不可用的能力。
#
# 收得进来的只有两种形状，别再加第三种：
#   ① 上游能力当前不在架（端点不存在 / 平台整体退役 / 暂时下架）；
