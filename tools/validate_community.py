...
TRIGGER_REAL_DELETION = {
    ...
    "optimize-skill-md",
    # 2026-08-31：服务端 skill.ai.imageGen（gpt-image-gen）因 ICP 备案被驳回（深圳网安要求
    # AI 深度合成业务提交算法接入协议/算法备案号+安全评估）整体下架，产品退回「数据服务+工具」
    # 形态——上游能力真没了，不是我们不想要。本仓没有任何包能接住「画张图/出图/文生图」这类词
    # （dby-image 是唯一真出图的包），迁进 dby-api 等于承诺一个已经不存在的能力。dby / dby-api
    # 的路由表与正文都已改成明确告知用户「生图能力已下线、无替代包」，而不是假装能路由过去。
    "dby-image",
}
