...
name: doubaoya
description: >-
  都爆鸭 (doubaoya) — 新媒体爆款选题 / 追热点 / 写脚本的 AI 工作搭子：教 agent 用一条 DOUBAOYA_API_KEY
  调 doubaoya.com 的公开 API，把散乱搜索变成能直接用的选题信号和脚本。覆盖各平台搜索 / 榜单 / 日报源 /
  账号分析 / 作品解析 / 违禁词，以及小红书封面·标题·笔记对标·低粉爆款·周榜、公众号 10 万+·原创榜·头部账号榜·
  追更发文列表·文旅短剧日报、视频号 AI 日报、全平台内容出海榜——这些原本各有一个薄壳 Skill，现统一由本 Skill 按意图路由
  （能力都还在架，只是不再各占一个包）。
