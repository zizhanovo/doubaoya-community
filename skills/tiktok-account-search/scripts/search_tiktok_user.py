...
零依赖（Python 3 标准库 urllib），按关键词搜索 TikTok 账号（博主），
按粉丝数从高到低展示。翻页只需递增 page，服务端内部把 page 映射成游标。

用法:
    python3 search_tiktok_user.py "<关键词>" [--page 1]

例如:
    python3 search_tiktok_user.py "fitness"
    python3 search_tiktok_user.py "美妆" --page 2
...
ENDPOINT = "https://doubaoya.com/api/apis/tiktok/search-user/call"


def main() -> int:
    parser = argparse.ArgumentParser(
        description="都爆鸭 · TikTok 账号搜索",
    )
