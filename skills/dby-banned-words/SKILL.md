...
version: 1.4.4
changelog: 执行层判据分三层（design D10）：确定性判据（三平台扇出/检出透传/命中词 span 粒度/无改写字段/无语境过滤）下沉到脚本层 tools/tests/test_check_multi.py，agent 用例 5→2 只留编排与解释，check 改打 tools.txt 不打摘要（实测原短语只在工具结果里、摘要里没有）；包行为零变化
compatibility: >-
