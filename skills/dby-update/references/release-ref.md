...
- `versions.json` 顶层 `ref` 是一个 release tag 名（`release-YYYYMMDD-HHMM`），由 `tools/stamp_versions.py` 写入。
...
1. `python3 tools/stamp_versions.py`——盖版本戳，同时把 `ref` 写进 `versions.json`（哈希一个没变就沿用上一次的 ref，不凭空造 tag 名）。
2. 提交、push。
...
回滚：删掉 `versions.json` 的 `ref` 字段即退回默认分支安装。
