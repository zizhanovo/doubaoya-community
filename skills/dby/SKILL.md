...
| 想按主题看同赛道公众号爆文、研究爆款规律 | `doubaoya` | 拉同主题爆文带阅读热度，找选题参考 |
...
| 要封面灵感 / 爆款封面参考 / 配色构图套路 | `doubaoya` | 同赛道爆款封面视觉套路 |
| 怕踩违禁词 / 要过审 / 合规改写 | `doubaoya` | 违禁词检测 + 产出可直接发布的过审版 |
...
> ⚠️ **规则外化边界**：下面这张图是**面向用户、由结论信号触发的分支导航**，只回答「拿到这个结果后，下一步可能走哪个 skill」，**不是**又一份会漂移的硬编码流程真相。选路那一侧的单一事实源是 `doubaoya` 包里的 `references/wechat-routing.json`（路由优先级、终态门、禁止误路由）；每一跳该不该继续往下走，以它和**用户说的终态**为准。
>
...
#### 写草稿阶段 —— 来自 `doubaoya` / `wechat-rewrite` / `dby-charter`

| 结论信号 | 推荐下一步 | 为什么 |
|---|---|---|
| 正文定稿，要起标题 | `doubaoya` | 标题定打开率，专项拉套路优化 |
| 要配封面 | `doubaoya` | 找同赛道封面配色 / 构图套路 |
| 怕正文踩违禁词 | `doubaoya` | 发布前过一遍合规、出过审版 |
