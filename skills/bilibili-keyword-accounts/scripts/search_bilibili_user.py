...
零依赖（Python 3 标准库 urllib），按关键词搜索 B 站账号（UP主），
支持综合排序或按粉丝数排序，返回账号列表（含粉丝数、等级、简介等）。

用法:
    python3 search_bilibili_user.py "<关键词>" [--order totalrank] [--page 1]

例如:
    python3 search_bilibili_user.py "数码"
    python3 search_bilibili_user.py "美食" --order fans --page 2
...
ENDPOINT = "https://doubaoya.com/api/apis/bilibili/search-user/call"


def main() -> int:
    parser = argparse.ArgumentParser(
        description="都爆鸭 · B站关键词搜账号",
    )
    parser.add_argument("keyword", help="搜索关键词（建议精简，2~6 字最佳）")
    parser.add_argument(
        "--order",
        default="totalrank",
        help='排序方式（可选，默认 "totalrank" = 综合排序；"fans" = 粉丝数排序）',
    )
    parser.add_argument(
        "--page",
        type=int,
