...
零依赖（Python 3 标准库 urllib），按分类拉抖音点赞飙升榜，
一次同时返回日榜（dailyRank）+ 周榜（weeklyRank），供主 Agent 抓正在起飞的内容。

用法:
    python3 fetch_content_surge.py [--type 美食] [--start-time YYYY-MM-DD]

    --type       内容分类，默认「美食」。
    --start-time 榜单日期 YYYY-MM-DD，默认昨天。
...
ENDPOINT = "https://doubaoya.com/api/apis/douyin/douyin-content-surge/call"
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="都爆鸭 · 抖音点赞飙升榜（日 + 周）",
    )
    parser.add_argument("--type", default="美食", help="内容分类（默认 美食）")
    parser.add_argument("--start-time", default=None, help="榜单日期 YYYY-MM-DD（默认昨天）")
    ...
    if args.start_time:
        ...
        start_time = (datetime.date.today() - datetime.timedelta(days=1)).isoformat()

    return call_api(api_key, {"type": args.type, "startTime": start_time})
