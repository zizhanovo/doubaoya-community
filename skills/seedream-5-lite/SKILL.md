...
### 1. 写提示词（可选定尺寸）
把用户需求转成清晰的图片提示词，必要时确定尺寸（默认 `1024x1024`）。
...
指定尺寸：
```bash
python3 "$SKILL_PATH/scripts/generate_image.py" "夏日柠檬汽水广告主视觉" --size 1024x1024
```
...
- 请求体：`{ "prompt": "<提示词>", "size": "1024x1024" }`
  - `prompt`：字符串，必填
  - `size`：字符串 WxH，可选（默认 `1024x1024`，由 `--size` 控制）
