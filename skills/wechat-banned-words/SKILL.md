...
description: 公众号违禁词检测与合规改写。扫描文案中的违禁词/敏感词，按广告法极限词、医疗、金融等类目标注风险，给出合规替换建议并产出一版过审版正文——这只是发布链上的合规这一环，过审后接着去起标题、配封面、排版存草稿。触发词：公众号违禁词、违禁词检测、敏感词、合规检测、过审、广告法、极限词。
---
...
> 过审版正文是**合规这一环的产物**，不是这篇稿子的终点——公众号还差标题、封面、排版和草稿箱，见下方[下一步](#下一步过审之后往哪走)。
...
过审版正文交出去**不代表这篇稿子完事了**——它还是一段纯文本，用户的公众号里还什么都没有。
交完合规结果，顺手把下一步接上：

| 接下来要做的事 | 交给 |
|---|---|
| 排版成公众号 HTML + 配封面 + 存进自己公众号草稿箱（**这条链的终点**） | `wechat-article-pipeline` |
| 已经有排好版的图文，只想推进草稿箱 | `wechat-draft-publish` |
| 标题还没起 / 想再优化标题 | `wechat-title` |
| 封面还没有 | `wechat-cover` |
| 违禁词太多，整段要回炉重写 | `wechat-rewrite` 或 `wechat-hot-write` |
| 拿不准先走哪一步 | `dby`（公众号飞轮的任务后导航） |

> 只在用户明确说「我只要检测，不发文」时才在这里停手；否则默认把过审版正文继续往排版/草稿箱推。
