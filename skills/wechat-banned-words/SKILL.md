...
脚本通过环境变量 `DOUBAOYA_API_KEY` 读取口令，口令形如 `dyh_…`。

获取步骤：

1. 打开 [doubaoya.com](https://doubaoya.com) 并 **登录**
2. 进入 **口令中心**
3. 点击 **生成口令**，复制得到的口令（形如 `dyh_xxxxxxxx`）
4. 配置到环境变量：
...
>
> 安全铁律：**口令绝不出现在对话、日志或任何输出里**。脚本只从环境变量读取，缺失时只提示"去哪拿"，不回显口令本身。
...
| 状态 | code | 含义 / 处理 |
|------|------|------|
| 401 | MISSING_API_KEY / UNAUTHORIZED | 口令缺失或无效 → 回 [拿钥匙](#拿钥匙口令) 重新生成并配置 |
...
未配置口令时脚本会直接提示去 doubaoya.com 生成口令，不会发起请求。
