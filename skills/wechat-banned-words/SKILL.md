...
description: 公众号违禁词检测与合规改写。扫描文案中的违禁词/敏感词，按广告法极限词、医疗、金融等类目标注风险，给出合规替换建议并产出一版过审版正文（纯文本；用户若还要排版好的公众号 HTML 或要存进草稿箱，另有 skill 接手）。触发词：公众号违禁词、违禁词检测、敏感词、合规检测、过审、广告法、极限词。
---
...
| 过审改写 | 本鸭基于建议产出一版完整的合规正文，替换处加粗，保持原文语气与排版 |
