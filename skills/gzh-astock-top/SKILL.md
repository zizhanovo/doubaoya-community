...
本鸭帮你一条命令摸清 A股领域的公众号头部大V——先按关键词把 A股公众号**发现**出来，再拉一遍每个账号的**数据画像**（平均阅读、健康度等），最后查一遍这些大V**当日发了什么**。三步在一个脚本里串好，吐一份合并 JSON，你直接拿去铺榜单。
...
| **账号画像对标** | 看各账号平均阅读 / 健康度 | 摸清谁是 A股内容头部 |
...
### 渲染榜单 + 一句洞察
从合并 JSON 里取数铺 Markdown 榜单：账号名（`accountName`）、平均阅读（`analysis` 里的 `avgReadCount`）、健康度（`healthScore`）、当日发文标题（`dailyPublish` 里的 `title`，渲染成**可点链接**）。字段做防御式读取，缺了留空、别脑补。结尾用本鸭口吻补一句 A股内容洞察：哪个大V在跑量、当日有没有共振话题。别堆套话。
...
脚本失败（步骤 1 失败或缺密钥）时向 stderr 打印 `[error] CODE: message` 并以退出码 1 结束。子步失败则进 `errors` 字段、不退出。常见情况：
