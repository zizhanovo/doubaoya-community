...
description: 小红书短剧笔记日报内容源 · 按关键词扫描小红书里的短剧爆款笔记，终端表格展示（标题链接/作者/点赞）并聚类成每日短剧选题日报。当用户需要小红书短剧爆款笔记、短剧种草、短剧笔记选题、短剧安利文案、追踪小红书短剧赛道时使用。
---
...
### 3. 渲染爆款表格
从 `data.items`（笔记数组）里取字段，铺成 Markdown 表格。字段做防御式读取——`title`（标题）/ `authorName`（作者）/ `likeCount`（点赞）可能缺失，缺了就留空，别报错。标题渲染成**可点链接**（指向笔记原文 URL）。

| 标题 | 作者 | 点赞 |
|------|------|------|
| [示例短剧爆款笔记](https://www.xiaohongshu.com/explore/xxx) | 某某追剧博主 | 24,000 |
...
- 返回信封：
  ```json
  {
    "success": true,
    "requestId": "...",
    "data": { "items": [ { "title": "...", "authorName": "...", "likeCount": 24000 } ] },
    "error": null
  }
  ```
