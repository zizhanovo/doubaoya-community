---
name: content-parse
description: 解析作品/解析链接 — 粘贴一条公开作品或文章链接，本鸭帮你解析作品详情/文章详情，拿到标题、作者、点赞/评论/转发等互动数据，并可顺手拆解爆款、分析「为什么火」（选题角度/钩子/时机）。适用于：解析作品、解析链接、作品详情、文章详情、拆解爆款、为什么火、内容解析。触发词：解析链接、链接解析、作品详情、扒文章。
---
...
2. 进入 **密钥中心**。
3. 点击 **生成密钥**，复制形如 `dyh_xxxxxxxx` 的密钥。
4. 配置到环境变量：
...
> **安全提示**：绝不要把密钥硬编码进代码、提示词、日志或输出文件；展示给用户时也不要回显完整密钥。本技能脚本从环境变量读取，永不打印密钥。
...
| HTTP | code | 含义 | 处理建议 |
|------|------|------|----------|
| 401 | `MISSING_API_KEY` | 没带密钥 | 检查 `DOUBAOYA_API_KEY` 是否已 export |
| 401 | `UNAUTHORIZED` | 密钥无效/已撤销 | 回密钥中心重新生成 |
...
**Q：我没有密钥怎么办？**
A：前往 [doubaoya.com](https://doubaoya.com) 登录 → 密钥中心 → 生成密钥（形如 `dyh_...`），再 `export DOUBAOYA_API_KEY`。
