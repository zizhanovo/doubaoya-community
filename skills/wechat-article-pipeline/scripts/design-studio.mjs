...
//   * AI 生封面/配图  ← ./gen-image.mjs         (generateImage，走密钥 DOUBAOYA_API_KEY)
//
...
生图走密钥 DOUBAOYA_API_KEY（缺失时页面会给出清晰错误）。绝不打印密钥。
`;
      ...
      if (req.method === "POST" && pathname === "/api/generate") {
        ...
        if (!process.env.DOUBAOYA_API_KEY) {
          return sendJson(res, 400, {
            error: "MISSING_KEY",
            message: "缺少环境变量 DOUBAOYA_API_KEY（doubaoya 密钥）。请先 export 后重启工作台再生图。",
          });
        }
