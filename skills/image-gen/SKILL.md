...
compatibility: >-
  ...
  ⚠️ 单次请求会在服务端同步等 1–2 分钟（最长 4 分钟）才返回，调用方（含反向代理与 agent
  的命令超时）必须留 ≥5 分钟，否则会在出图完成前被掐断——服务端超时会退款，客户端先断不会。
---
...
> ⏳ **这是异步慢操作**：图片在服务端生成，通常 **1–2 分钟**、最长 **4 分钟**（服务端上限 240 秒），单次请求内一气呵成返回——**无需客户端轮询**，发一次 POST 静等结果即可。
>
> 🔴 **客户端超时必须晚于服务端上限**，留 ≥5 分钟。顺序反了后果不对称：服务端超时会退款，客户端提前放弃**不会**——请求照样在服务端跑完、照样扣费，而你只看到一句「超时」。本仓脚本已按 300 秒设。
...
```
脚本会先在 stderr 提示「已提交，服务端生成中」，然后**等待 1–2 分钟（最长 4 分钟）**直到结果返回，把成功信封里的 `data` 以 JSON 打到 stdout。
...
- **异步慢操作**：服务端通常 1–2 分钟、最长 4 分钟内完成生成并在本次请求里返回，无需轮询。
- 返回信封：
...
- **慢是正常的**：1–2 分钟是预期、4 分钟才是上限，别因为慢就判定失败或重复调用——重复调用才会重复扣费。
