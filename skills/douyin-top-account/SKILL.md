...
description: 抖音影响力账号榜 · 按日期拉抖音最具影响力账号 TOP 榜，帮你看清哪些大号在领跑、哪个赛道最热、当下的流量风向。当用户需要抖音影响力榜、抖音大号排行、抖音头部账号、抖音影响力指数、抖音流量风向、抖音账号榜单时使用。触发词：抖音影响力榜、抖音大号排行、抖音头部账号、抖音榜单、抖音风向、影响力指数。
---
...
### 3. 渲染榜单表格
从 `data.items` 里取 **rank（名次）**、**accountName（账号名）**、**influenceScore（影响力指数）** 等字段（防御式读取，缺了留空），铺成 Markdown 表，按名次升序：

| 名次 | 账号 | 影响力指数 |
|------|------|----------|
| 1 | 某某账号 | xxxx |

> 影响力指标读 `influenceScore` 字段，对外展示统一叫「影响力指数」。
...
- 返回信封：
  ```json
  { "success": true, "requestId": "...", "data": { "items": [ { "rank": 1, "accountName": "...", "influenceScore": "..." } ] }, "error": null }
  ```
