...
零依赖（Python 3 标准库 urllib），按一批关键词批量扫描快手平台的
AI 创作爆款视频，用于做每日 AI 快手选题日报。

用法:
    python3 fetch_ks_ai_feed.py "<关键词1>" ["<关键词2>" ...] [--page N] [--size N]
...
ENDPOINT = "https://doubaoya.com/api/apis/kuaishou/kuaishou-ai-feed/call"


def main() -> int:
    parser = argparse.ArgumentParser(
        description="都爆鸭 · AI 快手日报内容源（批量关键词）",
    )
    parser.add_argument(
        "keywords",
        nargs="+",
        help="AI 方向关键词，可传多个（如 AI教程 ChatGPT 大模型）",
    )
    parser.add_argument(
        "--page",
        type=int,
    ...
    payload = json.dumps(
        {"keywords": args.keywords, "pageNum": args.page, "pageSize": args.size}
    ).encode("utf-8")
