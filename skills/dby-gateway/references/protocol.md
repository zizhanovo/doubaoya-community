...
6. **报错怎么办**（`HTTP` / `error.code`）：
   ...
   403 `PLAN_LIMIT_EXCEEDED` → 撞到**账号套餐的权益上限**了（可绑公众号数 / 随手记条数 / 本月存草稿次数 /
     自定义主题数 / 审稿台与主题 studio 两道功能门），**与钥匙、余额、点数都无关**——别换钥匙重试，别套 402 的余额话术。
     `error.extra` 带着 `dimension`（哪一维）、`plan`（当前档位）、`used` / `limit`（已用 / 上限）、
     `helpUrl`（账户页套餐区块的绝对地址）——**原样念给用户**：「你当前是 `plan` 档，`dimension` 已用 `used` / `limit`，
     已有内容不受影响；要放开这一项到 `helpUrl` 升级套餐」。**同一入参不重试**；这是终止态。
   503 `CAPABILITY_UNAVAILABLE` → **别重试**，换能力或如实告知；
   502 `PROVIDER_FAILED` → 上游临时失败，**额度已自动退回**，可以直接重试，**最多 3 次**。
   （无 HTTP 状态）`CONNECT_FAILED` → **连接从未建立，请求根本没发出去**。它不是服务端返回的错误码，
     是客户端在 TCP / TLS 阶段就失败后自己贴的标签：服务端没收到、没执行、**没扣点**，重试免费且安全。
     但先确认本机真能出网（`curl -s -o /dev/null -w '%{http_code}' https://doubaoya.com/api/health` 应回 200）——
     容器 / 沙箱的出站域名白名单会让重试永远是同一个结果，那要放行域名或换环境，不是多试几次的事。
     🔴 **别转述成「都爆鸭返回了错误」，也别说「可能被扣了点」**：两句都是错的，而且会把用户引去问服务端。
     🔴 **`curl` 通、脚本不通 ≠ 玄学**：受管网络里域名是代理替你解析的。本包**会**读
     HTTPS_PROXY / HTTP_PROXY / ALL_PROXY 自己走代理（遵守 NO_PROXY，只认 http(s) 代理），
     所以还看到解析失败，多半是那几个变量没传进当前进程（agent / 容器拉起子进程时最容易丢），
     或被 NO_PROXY 豁免了。先 `echo` 一眼那几个变量，别去查钥匙、余额或入参。
   （无 HTTP 状态）`NETWORK_ERROR` → 与上一条相反：连上之后才断，请求**可能已经送达并计费**，
     按超时那条保守处理——先核实有没有扣点，别直接重试。
   分类只有两种：**瞬时**（429、502、网络超时）才重试；**其余全是终止**（400 / 401 / 402 / 403 / 404 / 422 / 503），重试只烧预算，改入参 / 换钥匙 / 补点 / 换能力才是出路。
   重试超预算一律停下，把 `error.code`、`message` 原文和 `requestId` 交给用户，不自己绕。
   🔴 只有上面这条 404 走「先更新再重试」，**别的错一律不许触发更新**——
   401 是钥匙问题、400 是入参问题、402 是余额问题、403 是套餐权益问题，更新 skill 一个都治不了，
