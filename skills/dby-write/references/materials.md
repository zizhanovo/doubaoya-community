...
S10 [留言]     「……原话……」  留言·《标题》2026-08-20（用户贴的截图）
S11 [记录]     「……原句……」  记录 · ki_xxx · 9/1（灵感库，`inspirations` 拉到）
```

没进单的细节不许进正文；进了单的，正文里那句话要能指回它的编号。

## 1. 用户当轮给的材料（免费）——含读者留言 / 私信原话
...
用户平时随手记进灵感库的东西：一句话、一个链接、一张截图或语音的转写。这是**用户自己的原料**，
...
- 没点名时默认拉近 30 天扫一眼：有与本篇相关的就入单，没有就**静默跳过**，不追问。
- 入单出处写「记录 · <id> · M/D」，**id 必须带**——建稿 / 提交新版时这些 id 要作 `sourceItemIds` 提交（血缘）。
- 返回里 `requested` ≠ `returned` = 点名的 id 里有不属于你的或已归档的，如实告诉用户少了哪几条，别猜。
- 🔴 灵感库为空 / 没相关的 → 说一句「你最近没记相关的东西」然后照常往下取，**不虚构来源**。
- 🔴 记录只是素材，不是选题指令：用户只是记了几条、没说要写 → 不因此起稿、不主动提醒。
- 🔴 读取免费、不计费；接口 401 且 key 确认已设 → 服务端还没升级到带灵感库读取的版本，照常走后面几层。

## 3. 号档案 + 范文（免费，第 1 步已拉到）
...
（带转折的真实经历 / 被访者原话）时，八层取完仍拿不到锚点 **必须停下来问用户**，
...
出处写「wiki 文件相对路径」。笔记里转述的第三方数据仍要在第 8 层核实一次。

## 7. 平台取数（计费）

只在用户点名、或前六层凑不出这篇要的事实时用，用前照 `dby-api` 的计费规矩说明：
...
用宿主自带的搜索 / 抓取工具，**只用来核实前六层已经拿到的事实**（数字还成不成立、产品还叫不叫这个名）。
...
八层取完仍没有一件**真发生过、有明确时间或后果**的事 —— **停下，不许开始写正文**。
按这个优先级走：
