...
### 1. 给账号
传一个**真实存在**的账号即可——**中文账号名**或**抖音号 ID** 都可以，系统会自动识别（中文名走账号名、否则走账号 ID）。**别凭空捏一个名字 / 抖音号来"试一下"**——查不到的账号只会返回 `NO_DATA`。手上没有确切账号时，先问用户要真实的账号名或抖音号。
...
> 只呈现接口真实返回的字段（`profile.*` 与 `items[].title/likeCount/workUrl`）。接口没返回的"播放率 / 完播率 / 某种指数"一律不要凭空补。
