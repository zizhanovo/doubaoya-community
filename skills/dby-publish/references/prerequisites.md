...
只想看排版效果、写/换主题的用户，**没有密钥、没绑公众号也能干活**：
...
| AI 生封面 / 生配图（含配图位置规划） | 见 `dby-image`（生图**花钱**） | 出图不在本包：`dby-image` 出好图落成本地文件，封面喂 `--cover`、配图以 `<img src>` 落进正文 |
...
> 纯本地不要密钥的那条路（`render-wechat-html.mjs`）仍然在，代价是**拿不到在线链接**
> ——在线链接只有走平台渲染（即 `pipeline.mjs`）才有。
> 注意单跑渲染器时 `--title` 会往正文顶部插一个 `<h1>`（本地预览用），那份产物别拿去发布——见 `SKILL.md` 「正文不要写标题」一节。
