...
零依赖（Python 3 标准库 urllib），按关键词扫描小红书里的
文旅 / 城市 / 景区方向爆款笔记，用于做文旅选题与对标。

用法:
    python3 fetch_culture_feed.py "<关键词>" [--page N] [--size N]
...
ENDPOINT = "https://doubaoya.com/api/apis/xiaohongshu/xiaohongshu-playlet-feed/call"


def main() -> int:
    parser = argparse.ArgumentParser(
        description="都爆鸭 · 小红书文旅笔记内容源（按关键词）",
    )
    parser.add_argument(
        "keyword",
        help="文旅方向关键词（必填，如 文旅 / 城市旅游 / 景区打卡）",
    )
    parser.add_argument(
        "--page",
        type=int,
