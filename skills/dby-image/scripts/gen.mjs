...
//
//   · 客户端超时必须 > 服务端 240s        → 走共享请求层的 INVOKE_TIMEOUT_MS(450s)
...
//
// 🔴 请求**不自己发**：仓内只有一份请求层（dby-api/scripts/lib/http.mjs），
//    校验闸 validate_single_request_layer 会拦下任何绕过它的 fetch。
//    2026-09 重建本包时这里从裸 fetch 改成了 request()——顺带白拿了统一的
//    超时判据、错误分类与 notice 转达。
//
// env:
//   DOUBAOYA_API_KEY  （必填）doubaoya 密钥。**绝不打印、绝不落文件。**
//   DOUBAOYA_BASE_URL （可选）默认 https://doubaoya.com
//
// 零依赖（Node ≥18）；接口调用经 dby-api 包的共享 CLI 库，需与它一起安装。
//
// 用法:
...
// 出图的服务端处理上限是 240s。客户端必须**大于**它：服务端超时会退款，客户端提前放弃不会
...
async function describe() {
  ...
  let data;
  try {
    data = await request(ctx, "GET", `/api/skills/${SLUG}`, { timeoutMs: 30_000 });
  } catch (e) {
    die(`拉契约失败：${(e && e.message) || String(e)}`);
  }
  const props = data?.inputContract?.jsonSchema?.properties || {};
  const live = Object.keys(props).sort();
  const expected = [...EXPECTED_FIELDS].sort();
  console.log(`单价 ${data.unitPrice} 点／次   必填 ${JSON.stringify(data.inputContract.jsonSchema.required)}`);
...
async function main() {
  ...
  if (refs.length > MAX_REFS)
    die(`参考图 ${refs.length} 张，超过上限 ${MAX_REFS} 张。\n` +
        `请让用户自己挑保留哪 ${MAX_REFS} 张，别替他选。（服务端也会拒，不会替谁截断。）`);
  ...
  let env;
  try {
    // 🔴 这里**故意没有重试**，共享请求层里也没有。服务端超时会退款，客户端放弃不会 ——
    //    重试就是为同一张图付两次钱。billable:true 让超时文案带上「先去核实有没有扣点」。
    env = await request(ctx, "POST", `/api/skills/${SLUG}/invoke`, {
      body,
    ...
    const hint = {
      ...
      TIMEOUT: "🔴 不要重试。这次可能已经扣费、图可能已经生成。如实告诉用户，由他决定是否再来一次。"
    }[code] || (e && e.remediation) || "看 message 改入参。";
  ...
  // 按信封里的 mime 定扩展名，别按 --out 的后缀猜：2026-09 实测文生图与改图**都是 png**
  // （上游自己定输出格式，我方不再指定 output_format）；写死任何一种都会在下一次换代时出错。
  const ext = { "image/jpeg": "jpg", "image/png": "png", "image/webp": "webp" }[img.mime] || "png";
