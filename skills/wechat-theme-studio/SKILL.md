...
主题存好只是把「以后每篇长什么样」定了下来，**这篇稿子还没发出去**。接着走：

| 接下来要做的事 | 交给 |
|---|---|
| 用这套主题排版正文 + 配封面 + 存进自己公众号草稿箱（**终点**） | `wechat-article-pipeline`（存回服务端的默认排版它会自动拉；本机主题文件用 `--theme` 指过去） |
| 已有排好版的图文，只想推进草稿箱 | `wechat-draft-publish` |
| 正文还没写 | `wechat-hot-write` |
| 正文没过违禁词 | `wechat-banned-words` |
