...
description: 抖音账号概况查询。输入抖音账号的 secUid（一串真实的稳定标识，形如 MS4wLjABAAAA…，不是昵称），调用都爆鸭接口返回账号档案（昵称、粉丝数、关注数）。当用户提到"抖音账号概况"、"抖音账号档案"、"抖音粉丝数"、"达人账号信息"时使用。注意：本接口为账号档案级（profile-only），不返回逐条作品列表；如需拆解某条具体作品，引导用户改用内容解析技能（tool/parse-content-detail）并提供作品链接。
---

# 抖音账号概况查询

> 本鸭一句话：丢给我一个抖音账号的 secUid，我先把这只账号的「家底」给你摸清楚——昵称、粉丝、关注，一锅端。具体哪条作品好，咱们再单独拆。

---

## 这只鸭子能干啥

这是都爆鸭社区库里的「抖音账号概况」技能。它做的是**账号档案**这一层：
...
> 本技能不做"昵称模糊搜索"，也**不接受**编造的 secUid。请提供一串**真实的** secUid（形如 `MS4wLjABAAAA…`）；如果手上只有昵称或抖音号，先去拿到真实 secUid，否则接口会返回校验/无数据错误。**绝不要凭空捏一个 id 来"试一下"。**
...
- **返回信封**：
  ...
  **先看 `success`**：为 `true` 才取 `data`；否则读 `error.code` / `error.message`。
  解析 `data.profile` 请**防御式**取值——核心字段是 `nickname` / `followerCount` / `followingCount`，缺了就显示"—"，别让流程崩。
...
🦆 这就是这只账号的底子。
（本技能给的是账号档案；想逐条拆解某条作品，把作品链接发我，
  我用内容解析技能 tool/parse-content-detail 单独给你拆。）
```

数字呈现建议：≥1 亿用"亿"，≥1 万用"w"（如 123.5w），<1 万用千分位（如 9,860）。

> ⚠️ **绝不编造**：`data.profile` 里没有的字段（例如逐条作品、互动明细、作品总数、某种"指数"）一律不要凭空生成。接口没给，就如实说"本接口未提供"，并引导去用内容解析技能。
...
- **不越界**：本技能只做账号档案。逐条作品解析 → 内容解析技能 `tool/parse-content-detail`。
