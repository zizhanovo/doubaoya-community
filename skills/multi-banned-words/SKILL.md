...
- **没拿到结果**：该平台条目里有 `error`，或 `content` 与 `originalContent` 都缺失/为 null。
  🔴 这时该平台一律标「检测失败」，**绝不能填成「合规」/「low」**——漏报比误报危险得多，
  读不到就如实说读不到，不替用户下合规结论。
...
3. 风险横向比较是本 Skill 的核心价值——务必让用户一眼看清「哪个平台命中最多、类别最重」。
4. 安全改写要同时满足所有平台口径：以命中最多的平台为基线消词。
5. 某平台失败时，照常输出其余平台结果，并在对照表标注该平台失败原因；
   🔴 **有平台检测失败时，末尾必须提示「该平台未完成检测，别当它通过」**，不要写成全平台可发。
6. 全平台都判定为「无命中」时：对照表如实标注，安全改写处写「原文已可全平台发布，无需改动」。
