...
version: 2.1.1
changelog: dby-image 已随服务端生图能力一起下线，摘掉对它的指针；出图 / 配图请求如实告知不可用，不再假装能路由过去
compatibility: >-
...
🖼 **生图能力已下线**（服务端 `skill.ai.imageGen` 因合规要求整体下架，dby-image 包同期退役），
不在本包也没有替代包。用户要图片时如实告知，可用你自己 agent 的生图工具，或用户自备现成图片。
...
| 拿到数据之后 | 去哪 |
|---|---|
| 要出图 / 配图 / 改图 | ⛔ 已下线，本仓不再提供生图能力；用你自己 agent 的生图工具，或让用户自备图片 |
