...
version: 3.0.0
changelog: description 从 1021 降到 957 字符，给硬限 1024 留余量：删触发词「写作」（与本包「只给数据、写正文归 dby-write」自相矛盾），另删字面重复与子串词面、压缩出图下架段措辞（8 个否定词面未动）；补 3 条写作类负例。过 codex 与 pi 双后端盲测：本包与 dby-write / dby-rewrite 正例 100% 未劣化、负例零误触
compatibility: >-
