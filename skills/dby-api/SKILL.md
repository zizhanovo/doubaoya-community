...
都爆鸭云端能力覆盖的这一片。写正文、排版、查违禁词都不在这里，各有专包；出图能力当前暂时下架。
...
🖼 **出图能力当前暂时下架**，不要调用已下架能力或旧包。未来是否恢复需重新评估，当前不承诺恢复时间；现阶段可用你自己 agent 的生图工具，或自备图片。
...
| 拿到数据之后 | 去哪 |
|---|---|
| 要出图 / 配图 / 改图 | ⛔ 当前暂时下架；未来是否恢复需重新评估，当前不承诺恢复时间。现阶段用你自己 agent 的生图工具，或让用户自备图片 |
