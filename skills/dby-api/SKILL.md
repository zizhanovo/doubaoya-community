...
version: 4.4.0
changelog: 共享请求层把「连不上」与「连上后断」拆成两个码：连接从未建立回 CONNECT_FAILED，明说请求没发出去、没扣点、重试安全；连上后才断的留在 NETWORK_ERROR，按可能已计费保守处置。混成一个码时调用方一律套「计费失败不重试」卡死，还会把本机连不上说成「都爆鸭返回了错误」
compatibility: >-
