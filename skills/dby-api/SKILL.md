...
🖼 **出图不在本包**：`dby-image` 是 owner，等待与重试纪律只写在那个包里，本文件不复述。
...
全在 `dby-gateway/references/protocol.md` 第 6 条，本文件不复述——复述必漂。
网关只回答「怎么把一次调用打出去」，不承接业务意图；该做的事走哪条能力看上面那张表。
