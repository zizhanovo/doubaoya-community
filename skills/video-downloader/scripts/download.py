...
零依赖（Python 3 标准库 urllib），解析抖音 / 小红书 / 快手 / B站等
平台的公开视频，返回无水印直链与作品信息。

用法:
    python3 download.py "<分享链接>"
...
ENDPOINT = "https://doubaoya.com/api/skills/video-downloader/invoke"


def main() -> int:
    parser = argparse.ArgumentParser(
        description="都爆鸭 · 短视频下载器（解析无水印直链）",
    )
    parser.add_argument(
        "url",
        help="作品分享链接（每次仅一个，必填）",
    )
    ...
    payload = json.dumps({"url": args.url}).encode("utf-8")
