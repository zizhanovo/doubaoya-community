...
description: AI 视频号信息源 · 按关键词扫描视频号平台 AI 方向的爆款作品，终端表格展示（标题/作者/点赞）并按互动量聚类成每日选题日报。当用户需要视频号 AI 日报、视频号每日 AI 热点、视频号 AI 爆款、视频号内容选题、做视频号 AI 赛道内容发现时使用。
---

# AI 视频号信息源（都爆鸭）

本鸭帮你按关键词扫一遍视频号上 AI 方向的爆款作品——把当下视频号互动最高的 AI 内容捞出来，终端铺成爆款表格，再帮你聚类成几个话题方向，攒成一份能直接用的视频号 AI 选题日报。
...
### 3. 渲染爆款表格
从 `data.items`（作品数组）里取字段，铺成 Markdown 表格。字段做防御式读取——`title`（标题）/ `authorName`（作者）/ `likeCount`（点赞）可能缺失，缺了就留空，别报错。**按互动量（点赞/分享/评论）从高到低排**。

> **重要：视频号平台规则限制，无法提供作品链接。** 因此标题**不要**渲染成可点链接，直接以纯文本展示。如用户想看原作品，可复制标题前往视频号搜索查看。

| 标题 | 作者 | 点赞 |
|------|------|------|
| 一个 AI 工具搞定全套素材 | 某某 AI 号 | 8,600 |

### 4. 聚类成日报 + 一句洞察
把这批作品按话题方向**聚类**（如 工具教程 / AI绘画 / 数字人 / 变现玩法），每个方向挑出代表爆款，组成一份简短日报。结尾用本鸭口吻补一句**选题洞察**：今天视频号 AI 圈哪个方向最热、哪个角度还有空位。别堆套话，别反问用户目的。
...
- 返回信封：
  ```json
  {
    "success": true,
    "requestId": "...",
    "data": { "items": [ { "title": "...", "authorName": "...", "likeCount": 8600 } ] },
    "error": null
  }
  ```
