...
零依赖（Python 3 标准库），按关键词 + 时间区间拉取同主题爆款文章作为写作素材，
返回阅读/在看/点赞等热度字段，打 JSON 到 stdout。
取数后由主 Agent 提炼标题套路、选题切口与正文结构（脚本只负责取数）。

用法:
    python3 hot_write.py "<关键词>" [--start YYYY-MM-DD] [--end YYYY-MM-DD]
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description="都爆鸭 · 公众号写作助手（拉同主题爆文做素材）",
    )
