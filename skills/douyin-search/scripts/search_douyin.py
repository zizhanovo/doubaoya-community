...
零依赖（Python 3 标准库 urllib），按关键词批量搜抖音爆款作品，
打 JSON 到 stdout，由主 Agent 铺成「标题/达人/点赞/作品链接」终端表格做选题洞察。

用法:
    python3 search_douyin.py "<关键词>" [--start-date YYYY-MM-DD] [--end-date YYYY-MM-DD]
...
ENDPOINT = "https://doubaoya.com/api/apis/douyin/search-work/call"


def main() -> int:
    parser = argparse.ArgumentParser(
        description="都爆鸭 · 抖音爆款搜索（按关键词批量取数）",
    )
    ...
    parser.add_argument(
        ...
        dest="start_date",
        default=None,
        help="起始发布日期 YYYY-MM-DD（可选，不传则不限）",
    )
    parser.add_argument(
        "--end-date",
        dest="end_date",
        default=None,
        help="截止发布日期 YYYY-MM-DD（可选，不传则不限）",
    )
