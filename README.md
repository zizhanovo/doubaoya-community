...
**`dby-image`（AI 生图与改图）已于 2026-09 恢复**：它曾在 2026-08-31 随服务端出图能力一起下架，
现在服务端换了上游供应商与模型重新上线，包也随之重建。它现在**必须与 `dby-api` 一起安装**
（请求走后者的共享请求层）。
...
| **dby-image** | AI 生图与改图：文生图 / 图生图 / 改图 / 主视觉 / 单独配一张插图，出图后落成本地文件给你路径（**慢操作**，通常 1–2 分钟；生图计费；需与 `dby-api` 一起安装） |
