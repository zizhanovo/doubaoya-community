...
**`dby-image`（AI 生图与改图）已下架，且复原了也用不了**：服务端 `skill.ai.imageGen` 生图能力
因合规要求（AI 深度合成内容备案）整体下线，不是包本身不想要了。本地副本仍会被归档、也能用复原
命令搬回来，但调用会直接 404——没有替代能力可以路由。需要图片时，请用你自己 agent 的生图工具，
或自备现成图片交给 `dby-publish` 走 `--cover` / 正文 `<img src>`。
...
| **dby-api** | 取数与创作总入口：一条 `DOUBAOYA_API_KEY` 调平台全部在架能力（热榜 / 爆文 / 账号 / 违禁词 / 生图 / 联网查证），按意图路由 |
